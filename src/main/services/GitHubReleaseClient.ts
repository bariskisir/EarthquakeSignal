/**
 * Retrieves published Earthquake Signal releases and downloads verified installers from GitHub.
 */
...
export const selectWindowsInstaller = (
  ...
  const expectedName = `earthquake-signal-${release.version}-windows-${releaseArchitecture}-setup.exe`
...
export default class GitHubReleaseClient {
  ...
  private async fetchLatestRelease(): Promise<GitHubRelease> {
    const response = await this.fetcher(RELEASES_API_URL, {
      headers: {
        ...
        'User-Agent': 'Earthquake-Signal-Desktop',
  ...
  private async parseApiRelease(response: Response): Promise<GitHubRelease> {
    ...
    return {
      version,
      name: parsed.name ?? `Earthquake Signal v${version}`,
  ...
  public async downloadInstaller(
    ...
    const response = await this.fetcher(asset.downloadUrl, {
      headers: { 'User-Agent': 'Earthquake-Signal-Desktop' },
      redirect: 'follow',
