...
const ru = {
  ...
  settings: {
    ...
    mapTileProvider: 'Стиль карты',
    mapTileProviderDescription: 'Выберите внешний вид карты для светлой и тёмной темы.',
    mapTileProviders: {
      esri: 'Esri Серый (Рекомендуется)',
      osm: 'OpenStreetMap Стандарт',
      carto: 'CARTO Светлая/Тёмная (требуется API-ключ)',
      google: 'Google Maps',
    ...
    mapTheme: 'Тема карты',
    mapThemeDescription: 'Выберите, как цвета карты адаптируются к теме.',
    mapThemeModes: {
      system: 'Система',
      light: 'Светлая',
      dark: 'Тёмная',
    ...
    mapSettings: 'Карта',
    mapApiKey: 'API-ключ карты',
    mapApiKeyDescription:
      'Требуется для некоторых провайдеров (напр. CARTO). Оставьте пустым, если не нужно.',
    mapApiKeyPlaceholder: 'API-ключ (необязательно)',
    ...
    telemetryCollection: 'Отправлять анонимную телеметрию использования',
    telemetryDescription:
      'При запуске приложения отправляются анонимный идентификатор установки, версия, платформа и язык интерфейса для подсчёта активных пользователей. Microsoft определяет примерные страну и город по IP-адресу подключения. Данные о землетрясениях не отправляются.',
